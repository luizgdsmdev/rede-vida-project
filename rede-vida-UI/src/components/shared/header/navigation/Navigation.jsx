    ...
    <>
      ...
      <div className="hidden lg:flex justify-end gap-8 items-center">
          <nav className="flex items-center gap-4">
              ...
              <Link className={navLinkClass} to="/location">
                {t('navigation.locations')}
              </Link>
              <Link className={navLinkClass} to="/requirements">
      ...
      <div className={`lg:hidden fixed top-[60px] left-0 right-0 bg-white/95 dark:bg-background-dark/95 backdrop-blur-3xl border-b border-[#f3e7e9] dark:border-[#3d2a2d] transition-all duration-300 ease-in-out ${isMenuOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-2'}`}>
          <div className="flex flex-col p-6 gap-6">
              <nav className="flex flex-col gap-4">
                  <Link className={`${navLinkClass} text-center`} to="/about" onClick={toggleMenu}>
                      {t('navigation.about')}
                  </Link>
                  <Link className={`${navLinkClass} text-center`} to="/location" onClick={toggleMenu}>
                      {t('navigation.locations')}
                  </Link>
                  <Link className={`${navLinkClass} text-center`} to="/requirements" onClick={toggleMenu}>
                      {t('navigation.requirements')}
                  </Link>
                  <Link className={`${navLinkClass} text-center`} to="/contact" onClick={toggleMenu}>
                      {t('navigation.contact')}
                  </Link>
              </nav>
